...
class VerificationViewController: BaseViewController,VPMOTPViewDelegate {
    ...
    @objc func Discoverotp(_ notification: Notification) {
        ...
        DispatchQueue.main.async() {
            if status == 0{
                ...
                NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: "DiscoverVerificationNotification"), object: nil)
                self.removeAllOverlays()
            }
        }
    }
    ...
    @IBAction func backBtnAction(_ sender: Any) {
        NotificationCenter.default.post(name: NSNotification.Name("signUpReset"), object: nil)
    ...
    @IBAction func continueBtnAction(_ sender: Any) {
        if !GetotpString.isEmpty {
            if Reachability.isConnectedToNetwork() == true {
                showWaitOverlay()
                Parsing().DiscoverVerification(UserId: user_Id, OTP: GetotpString)
                NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: "DiscoverVerificationNotification"), object: nil)
                NotificationCenter.default.addObserver(self, selector: #selector(self.Discoverotp), name: NSNotification.Name(rawValue: "DiscoverVerificationNotification"), object: nil)
            }else {
                let test =  SwiftToast(
    ...
    @IBAction func resendBtnAction(_ sender: Any) {
        showWaitOverlay()
        Parsing().DiscoverResendotp(Email: str_email)
        NotificationCenter.default.addObserver(self, selector: #selector(self.ResendOTPAction), name: NSNotification.Name(rawValue: "DiscoverResendOtpNotification"), object: nil)
    }
    /*
    // MARK: - Navigation
