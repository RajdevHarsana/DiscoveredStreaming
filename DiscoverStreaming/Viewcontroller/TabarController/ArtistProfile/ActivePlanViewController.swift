...
class ActivePlanViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,CLLocationManagerDelegate {
    
    
    @IBOutlet weak var activePlanCollectionView: UICollectionView!
...
//        let proStatus = Int(status)
        DispatchQueue.main.async() {
            if status == 0{
                ...
                self.activePlanCollectionView.reloadData()
    ...
    override func viewDidLoad() {
        ...
        self.activePlanCollectionView.delegate = self
        self.activePlanCollectionView.dataSource = self
...
//        activePlanCollectionView.collectionViewLayout = LiberaryLayOut
            ...
            if userLocation.coordinate.latitude>0  {
                ...
                if !isCall
                {
                    if Reachability.isConnectedToNetwork(){
                        showWaitOverlay()
                        Parsing().DiscoverFeaturedCurrentPlan(user_Id: user_id, PlanTypeid: 0, PlanType: "featured_songs")
                         NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: "DiscoverFeaturedCurrentPlan"), object: nil)
                         NotificationCenter.default.addObserver(self, selector: #selector(self.DiscoveredFeaturedSongPlanAction), name: NSNotification.Name(rawValue: "DiscoverFeaturedCurrentPlan"), object: nil)
                    }else {
                        let test =  SwiftToast(
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as! ActivePlanCell
...
//        dict = response.object(forKey: indexPath.row) as! NSDictionary
        ...
        cell.ActivePlanName_lbl.text = response.value(forKey: "package_name") as? String ?? ""
