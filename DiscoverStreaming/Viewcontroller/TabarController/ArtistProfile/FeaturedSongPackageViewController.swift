//
//  FeaturedSongPackageViewController.swift
//  DiscoverStreaming
//
...
class FeaturedSongPackageViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {

    @IBOutlet weak var songsPackageCollectionView: UICollectionView!
    ...
    @objc func DiscoveredFeaturedSongPlanAction (_ notification: Notification){
        ...
        DispatchQueue.main.async() {
            if proStatus == 0{
                ...
                self.songsPackageCollectionView.reloadData()
                self.removeAllOverlays()
            }
        }
    }
    ...
    override func viewDidLoad() {
        ...
        self.songsPackageCollectionView.delegate = self
        self.songsPackageCollectionView.dataSource = self
...
//        packageForId = defaults.value(forKey: "ArtistID") as? Int ?? 0
        ...
        if Reachability.isConnectedToNetwork() == true {
            showWaitOverlay()
            Parsing().GetFeaturedSongPlanList()
            NotificationCenter.default.addObserver(self, selector: #selector(self.DiscoveredFeaturedSongPlanAction), name: NSNotification.Name(rawValue: "GetFaeturedSongPlanListNotification"), object: nil)
        }else {
            let test =  SwiftToast(
        ...
        songsPackageCollectionView.collectionViewLayout = LiberaryLayOut
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as! FeaturedSongPackageCell
        var dict = NSDictionary()
        dict = response.object(at: indexPath.row) as! NSDictionary
        
        cell.songPlanName_lbl.text = dict.value(forKey: "package_name") as? String ?? ""
