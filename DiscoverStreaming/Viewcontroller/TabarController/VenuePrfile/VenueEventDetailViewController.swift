//
//  VenueEventDetailViewController.swift
//  DiscoverStreaming
//
...
class VenueEventDetailViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate,GMSMapViewDelegate {
   ...
   @IBOutlet weak var performarTableview: UITableView!
    ...
    @IBOutlet weak var totalNoOfTickets_lbl: UILabel!
    @IBOutlet weak var soldTickets_lbl: UILabel!
    @IBOutlet weak var soldOut_btn: UIButton!
    @IBOutlet weak var scanTicket_btn: UIButton!
    @IBOutlet weak var scanTicketWidthCons: NSLayoutConstraint!
    ...
    @IBOutlet weak var numOfTicketsTopCons: NSLayoutConstraint!
    @IBOutlet weak var soldTicketsTopCons: NSLayoutConstraint!
            ...
            DispatchQueue.main.async() {
                if status == 0{
                    ...
                    if showdata == 1 {
                        ...
                        self.numOfTicketsTopCons.constant = 8
                        self.soldTicketsTopCons.constant = 8
                        ...
                        self.totalNoOfTickets_lbl.isHidden = true
                        self.soldTickets_lbl.isHidden = true
                        self.viewVenueDetailTopCons.constant = 25
                    }else {
                        
                        self.maxnooftic = response.value(forKey: "no_of_ticket") as? Int
                        var totalno = String()
                        totalno = String(self.maxnooftic)
                        self.totalNoOfTickets_lbl.text = "Total number of Tickets : \(totalno)"
                        
                        var soldTicket:Int!
                        soldTicket = response.value(forKey: "ticket_booked") as? Int
                        self.soldTickets_lbl.text = "Sold Tickets : \(String(soldTicket))"
                        ...
                        self.totalNoOfTickets_lbl.isHidden = false
                        self.soldTickets_lbl.isHidden = false
                        self.soldOut_btn.isHidden = true
                        self.scanTicketWidthCons.constant = self.screenWidth - 40
                        self.viewVenueDetailTopCons.constant = 25
                    }
...
//                    self.addresslbl.text = self.vaenuedetail.value(forKey: "address") as? String
                    self.addressven = self.vaenuedetail.value(forKey: "address") as? String
...
//                        self.mapviewnew.animate(with: updatedCamera)
        ...
        override func viewDidLoad() {
            ...
            self.performarTableview.delegate = self
            self.performarTableview.dataSource = self
            self.soldOut_btn.layer.cornerRadius = 20
            self.scanTicket_btn.layer.cornerRadius = 20
...
//            mapviewnew.delegate = self
    ...
    @IBAction func soldOut_btn(_ sender: Any) {
        
        
        let alertPrompt = UIAlertController(title: "", message: "Are you sure you want to declare this event as Sold Out?", preferredStyle: .alert)
        let confirmAction = UIAlertAction(title: "YES", style: UIAlertAction.Style.default, handler: { (action) -> Void in
            
            if Reachability.isConnectedToNetwork() == true {
                self.showWaitOverlay()
                Parsing().DiscoverTicketSoldOut(UserId: self.userId, EventId: self.eventID, VenueId: self.venue_id)
            NotificationCenter.default.addObserver(self, selector: #selector(self.DiscoverTicketsSoldAction), name: NSNotification.Name(rawValue: "DiscoverTicketsSold"), object: nil)
             }else {
                let test =  SwiftToast(
                    text: "Internet Connection not Available!",
                    textAlignment: .center,
    ...
    @IBAction func scanTicket_btn(_ sender: Any) {
//        let vc = storyboard?.instantiateViewController(withIdentifier: "QRScannerController") as! QRScannerController
//        navigationController?.pushViewController(vc, animated: true)
    }
    
    @objc func DiscoverTicketsSoldAction(_ notification: Notification) {
    ...
    DispatchQueue.main.async() {
        if status == 0{
            ...
            if Reachability.isConnectedToNetwork() == true {
                self.showWaitOverlay()
                Parsing().DiscoverEventDetail(UserId: self.userId, EventId: self.eventID, ArtistId: 0)
            NotificationCenter.default.addObserver(self, selector: #selector(self.DiscoverEventDetailAction), name: NSNotification.Name(rawValue: "DiscoverEventDetail"), object: nil)
             }else {
                let test =  SwiftToast(
                    text: "Internet Connection not Available!",
