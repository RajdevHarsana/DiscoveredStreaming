//
//  VenueRevenueViewController.swift
//  DiscoverStreaming
//
...
class VenueRevenueViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate {

    @IBOutlet weak var Searchtxt: UISearchBar!
    @IBOutlet weak var short_btn: UIButton!
    @IBOutlet weak var myRevenueTable: UITableView!
    
    
    @IBOutlet weak var shortView1: UIView!
    @IBOutlet weak var shortViewHeightCons: NSLayoutConstraint!
    @IBOutlet weak var revenueListHieghtCons: NSLayoutConstraint!
...
//        slooooooooooot = 0
        self.shortType = ""
        self.myRevenueTable.delegate = self
        self.myRevenueTable.dataSource = self
        self.revenueVenueListApi()
        self.shortView1.isHidden = true
        self.revenueListHieghtCons.constant = 571
        self.shortViewHeightCons.constant = 0
        let myColor : UIColor = UIColor.gray
        self.shortView1.layer.borderWidth = 1
        self.shortView1.layer.borderColor = myColor.cgColor
        self.shortView1.layer.cornerRadius = 10
    ...
    func revenueVenueListApi(){
        ...
        if Reachability.isConnectedToNetwork() == true{
            ...
            parameterDictionary.setValue(DataManager.getVal(self.venueid), forKey: "venue_id")
            ...
            DispatchQueue.main.async(execute: {
                ...
                if status == "0" {
                    ...
                    self.myRevenueTable.reloadData()
                }else{
...
//            self.slooooooooooot = 0
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! RevenueEventCell
        var dict = NSDictionary()
        dict = response.object(at: indexPath.row) as! NSDictionary
        cell.revenueEvent_Img.sd_setImage(with: URL(string: (dict.value(forKey: "event_image") as? String)!), placeholderImage: UIImage(named: "Group 4-1"))
        cell.revenueEventName_Lbl.text = dict.value(forKey: "event_title") as? String
        cell.revenueEventDate_Lbl.text = dict.value(forKey: "event_date") as? String
        var price = String()
        price = dict.value(forKey: "total_revenue") as? String ?? ""
        cell.revenueEventPrice_Lbl.text =  "$ " + price
...
//    }
    ...
    @IBAction func short_btn_Action(_ sender: UIButton) {
        self.shortView1.isHidden = false
        self.revenueListHieghtCons.constant = 423
