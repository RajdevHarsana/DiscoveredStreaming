...
class RecentlyAddedViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate,AVAudioPlayerDelegate {
...
//            parameterDictionary.setValue(DataManager.getVal(""), forKey: "as_type_id")
            parameterDictionary.setValue(DataManager.getVal("recently_added"), forKey: "list_type")
...
//                self.totalSongs = songCount
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tableView == recentlyAddedTableview {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
     if tableView == recentlyAddedTableview {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! RecentlyAddedListCell
        var dict = NSDictionary()
        dict = response.object(at: indexPath.row) as! NSDictionary
        cell.songNameLbl.text = dict.value(forKey: "song_name") as? String
        cell.songTypeLbl.text = dict.value(forKey: "song_type") as? String
    ...
    @objc func handleTap(_ sender:UIButton) {
        if IS_IPHONE_5S {
            let buttonPosition = sender.convert(CGPoint.zero, to: recentlyAddedTableview)
            myindexpath = recentlyAddedTableview.indexPathForRow(at: buttonPosition)! as NSIndexPath
            let cell = recentlyAddedTableview.dequeueReusableCell(withIdentifier: "cell", for: myindexpath as IndexPath) as! RecentlyAddedListCell
