//
//  FeturedSongViewController.swift
//  DiscoverStreaming
//
...
class FeturedSongViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate,AVAudioPlayerDelegate {

    @IBOutlet weak var feturedsongTableview: UITableView!
...
//                      self.feturedsongTableview.reloadData()
                  }
                  else{
...
//                SwAlert.showNoActionAlert(SwAlert().AppAlertTitle, message:str_message , buttonTitle: "OK")
    ...
    func songListApi(){
        ...
        if Reachability.isConnectedToNetwork() == true{
            ...
            parameterDictionary.setValue(DataManager.getVal("featured_song"), forKey: "list_type")
            ...
            parameterDictionary.setValue(DataManager.getVal(0), forKey: "list_type_id")
...
//                self.totalSongs = songCount
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tableView == feturedsongTableview {
    ...
    if tableView == feturedsongTableview {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! FeturedSongCell
        var dict = NSDictionary()
        dict = response.object(at: indexPath.row) as! NSDictionary
        cell.featuredsongNameLbl.text = dict.value(forKey: "song_name") as? String
        cell.featuredsongTypeLbl.text = dict.value(forKey: "song_type") as? String
        cell.featuredsongDurationLbl.text = dict.value(forKey: "duration") as? String
        cell.featuredsongImage.layer.cornerRadius = 5
        cell.featuredsongImage.sd_setImage(with: URL(string: (dict.value(forKey: "song_image") as? String)!), placeholderImage: UIImage(named: "Group 4-1"))
        cell.report_btn.tag = indexPath.row
        cell.report_btn.addTarget(self, action: #selector(handleTap(_:)), for: .touchUpInside)
        return cell
    }else {
    ...
    @objc func handleTap(_ sender:UIButton) {
        if IS_IPHONE_5S {
            let buttonPosition = sender.convert(CGPoint.zero, to: feturedsongTableview)
            myindexpath = feturedsongTableview.indexPathForRow(at: buttonPosition)! as NSIndexPath
            let cell = feturedsongTableview.dequeueReusableCell(withIdentifier: "cell", for: myindexpath as IndexPath) as! FeturedSongCell
            cell.report_btn.tag = myindexpath.row
            var dict = NSDictionary()
            dict = response.object(at: myindexpath.row) as! NSDictionary
