...
class RecentlyPlayedViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate,AVAudioPlayerDelegate {
...
//                    arr_data.removeAllObjects()
    ...
    func songListApi(){
            ...
            if Reachability.isConnectedToNetwork() == true{
                ...
                parameterDictionary.setValue(DataManager.getVal("recently_played"), forKey: "list_type")
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tableView == recentlyPlayedTableview {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
     if tableView == recentlyPlayedTableview {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! RecentlyPlayedListCell
        var dict = NSDictionary()
        dict = response.object(at: indexPath.row) as! NSDictionary
        cell.songNameLbl.text = dict.value(forKey: "song_name") as? String
        cell.songTypeLbl.text = dict.value(forKey: "as_type_name") as? String
        cell.songDurationLbl.text = dict.value(forKey: "duration") as? String
        cell.songImge.layer.cornerRadius = 5
        cell.songImge.sd_setImage(with: URL(string: (dict.value(forKey: "song_image") as? String)!), placeholderImage: UIImage(named: "Group 4-1"))
        ...
        cell.songReport_btn.tag = indexPath.row
        cell.songReport_btn.addTarget(self, action: #selector(handleTap(_:)), for: .touchUpInside)
        return cell
     }else{
    ...
    @objc func handleTap(_ sender:UIButton) {
        if IS_IPHONE_5S {
            let buttonPosition = sender.convert(CGPoint.zero, to: recentlyPlayedTableview)
            myindexpath = recentlyPlayedTableview.indexPathForRow(at: buttonPosition)! as NSIndexPath
            let cell = recentlyPlayedTableview.dequeueReusableCell(withIdentifier: "cell", for: myindexpath as IndexPath) as! RecentlyPlayedListCell
            cell.songReport_btn.tag = myindexpath.row
            var dict = NSDictionary()
            dict = response.object(at: myindexpath.row) as! NSDictionary
