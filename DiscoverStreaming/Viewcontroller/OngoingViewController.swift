...
import CoreLocation
import SwiftToast
class OngoingViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate {
        ...
        if userLocation.coordinate.latitude>0  {
            ...
            if !isCall
            {
                if Reachability.isConnectedToNetwork() == true {
                    showWaitOverlay()
                    Parsing().DiscoverEventListing(UserId: user_Id, EventStatus: "ongoing", ArtistId: venueid, Lat: str_lat as String?, Long: str_long as String?, Offset: 0)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! OngoingCell
