//
//  BandsFollowViewController.swift
//  DiscoverStreaming
//
...
class BandsFollowViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate {

    @IBOutlet weak var bandFollowTbaleview: UITableView!
    ...
    @objc func DiscoverFollowListAction(_ notification: Notification) {
        ...
        DispatchQueue.main.async() {
            if status == 0{
                ...
                self.bandFollowTbaleview.reloadData()
    ...
    @objc func FollowArtistAction(_ notification: Notification) {
        ...
        DispatchQueue.main.async() {
            if status == 0{
                ...
                Parsing().DiscoverGetFollowingList(UserId: self.userid, FollowType: "band", Lat: self.str_lat as String?, Long: self.str_long as String?, Offset: 0)
        ...
        if userLocation.coordinate.latitude>0  {
            ...
            if !isCall
            {
                if Reachability.isConnectedToNetwork(){
                    showWaitOverlay()
                    Parsing().DiscoverGetFollowingList(UserId: userid, FollowType: "band", Lat: str_lat as String?, Long: str_long as String?, Offset: 0)
                    NotificationCenter.default.addObserver(self, selector: #selector(self.DiscoverFollowListAction), name: NSNotification.Name(rawValue: "DiscoverGetFollowingList"), object: nil)
                } else {
                    let test =  SwiftToast(
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! BandsFollowCell
        ...
        var rate = Float()
        rate = Float(rating1)
        cell.bandRatingView.rating = Float(truncating: NSNumber(value: rate))
        var ratingper:Int!
        ratingper = dict.value(forKey: "rating_percentage") as? Int
        var rateper = String()
        rateper = String(ratingper)
        cell.bandRatePer.text = rateper + "%"
    ...
    @objc func HandleTap(_ sender:UIButton)
    {
        ...
        showWaitOverlay()
        Parsing().DiscoverArtistFollowUnfollow(user_Id: userid, FollowId: followid, Follow_Type: "band")
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: "DiscoverArtistFollowUnfollow"), object: nil)
