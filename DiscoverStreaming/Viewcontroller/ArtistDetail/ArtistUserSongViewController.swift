//
//  ArtistUserSongViewController.swift
//  DiscoverStreaming
//
...
class ArtistUserSongViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate,AVAudioPlayerDelegate {

    @IBOutlet weak var artistSongTable: UITableView!
    @IBOutlet weak var songImage: UIImageView!
    @IBOutlet weak var songBtnHeightCons: NSLayoutConstraint!
    ...
    var APICall = Bool()
    var newSong = String()
    var artistAccountID = String()
    var featuredSongStatus = Int()
...
//            self.venueRefreshControl.attributedTitle = NSAttributedString(string: "Pull to refresh")
            self.songRefreshControl.addTarget(self,action: #selector(self.refreshVenueList),for: .valueChanged)
            self.artistSongTable.addSubview(self.songRefreshControl)
        }
    }
        ...
        arid = defaults.integer(forKey: "ArtistID")
        str_lat = defaults.value(forKey: "LAT") as? String
        str_long = defaults.value(forKey: "LONG") as? String
        
        if artistUserId == user_Id {
            self.songImage.isHidden = false
            self.songBtnHeightCons.constant = 70
            
        }else {
            self.songImage.isHidden = true
            self.songBtnHeightCons.constant = 0
        }
        slooooooooooot = 0
        APICall = true
    ...
    @objc func refreshVenueList(sender: UIRefreshControl) {
       // Code to refresh table view
        self.artistSongTable.reloadData()
    ...
    override func viewWillAppear(_ animated: Bool) {
        defaults = UserDefaults.standard
        newSong = defaults.string(forKey: "Bool") ?? ""
        if newSong == "true"{
        songListApi()
        }
    }
    ...
    func songListApi(){
            ...
            if Reachability.isConnectedToNetwork() == true{
                ...
                parameterDictionary.setValue(DataManager.getVal(arid), forKey: "as_type_id")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! ArtistUserSongCell
        var dict = NSDictionary()
        dict = response.object(at: indexPath.row) as! NSDictionary
        self.artistAccountID = dict.value(forKey: "artist_account_id") as! String
    ...
    @IBAction func addnewSongBtnAction(_ sender: Any) {
        ...
        vc.ARTaccID = self.artistAccountID
