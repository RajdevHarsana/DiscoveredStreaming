//
//  ArtistUserDetailViewController.swift
//  DiscoverStreaming
//
...
class ArtistUserDetailViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,FloatRatingViewDelegate {
    ...
    @objc func DiscoverArtistDetailAction(_ notification: Notification) {
        ...
        DispatchQueue.main.async() {
            if status == 0{
                ...
                self.ArtistName.text = response.value(forKey: "artist_name") as? String
                self.ArtistGmail.text = response.value(forKey: "email") as? String
                self.artistAccountID = response.value(forKey: "artist_account_id") as? String
                var str_city = String()
...
//                if self.communityArray == nil {
//                    self.viewHeightCons.constant = 0
//                    self.viewAllBtn.isHidden = true
//                }else {
//                    self.viewHeightCons.constant = 290
//                    self.viewAllBtn.isHidden = false
//                    self.communityImageView.sd_setImage(with: URL(string: (dict.value(forKey: "post_image") as? String)!), placeholderImage: UIImage(named: "Artist_img_3"))
//                    self.postTitleLbl.text = dict.value(forKey: "post_title") as? String
//                    self.postDescription.text = dict.value(forKey: "description") as? String
...
//        messageBtn.layer.cornerRadius = 18
        ...
        showWaitOverlay()
        Parsing().DiscoverArtistDetail(user_Id: userId, Artist_id: ArtistId, ArtistUserId: artistUserId)
    ...
    @IBAction func viewAllbvtnAction(_ sender: Any) {
        let defaults = UserDefaults.standard
        defaults.set(self.arid, forKey: "IDAR")
        defaults.synchronize()
    ...
    @IBAction func followBtnAction(_ sender: Any) {
        if Reachability.isConnectedToNetwork() == true {
        showWaitOverlay()
        Parsing().DiscoverArtistFollowUnfollow(user_Id: userId, FollowId: ArtistId, Follow_Type: "artists")
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: "DiscoverArtistFollowUnfollow"), object: nil)
