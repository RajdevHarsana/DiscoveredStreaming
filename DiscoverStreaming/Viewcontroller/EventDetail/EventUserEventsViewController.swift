//
//  EventUserEventsViewController.swift
//  DiscoverStreaming
//
...
class EventUserEventsViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate {
        ...
        if userLocation.coordinate.latitude>0  {
            ...
            if !isCall
            {
                if Reachability.isConnectedToNetwork(){
                    showWaitOverlay()
                    Parsing().DiscoverEventListing(UserId: userId, EventStatus: "band", ArtistId: BandId, Lat: str_lat as String?, Long: str_long as String?, Offset: 0)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! EventUserEventCell
