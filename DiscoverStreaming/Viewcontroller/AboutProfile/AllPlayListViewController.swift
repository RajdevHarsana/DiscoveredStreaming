...
class AllPlayListViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    //        self.downloadSongLbl.text = str_like + " " + "Songs"
           ...
           // self.purchaseSongLbl.text = str_download + " " + "Songs"
            DispatchQueue.main.async() {
                if status == 0{
                    self.musicsTableview.isHidden = true
                    ...
                    if self.response.count == 0 {
                        ...
                        self.musicsTableview.isHidden = false
